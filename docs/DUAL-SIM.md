...
Remix 在“更多功能”页提供双卡控制和按卡流量统计页面：

| 页面 | 路径 | 作用 |
|------|------|------|
| 双卡管理 | `ui/functions/sim-switch.html` | 驻网模式、智能切换、当前数据卡和卡槽状态 |
| 流量管理 | `ui/functions/sim-traffic.html` | 按 ICCID 持久统计、套餐额度和每月重置日 |

双卡管理入口要求厂商接口返回 `support_dual_sim=1`。流量管理入口只要求配套
`zwrt-datad` 在 `/state` 中提供 `sim_traffic.available=true`：单卡设备只显示自插卡，
双卡设备仅在内置卡已启用时显示内置卡统计和套餐选择。
